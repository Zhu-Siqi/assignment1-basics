...
from cs336_basics.config import config_owt
...
TRAIN_PATH = 'data/owt-train.npy'
VALID_PATH = 'data/owt-valid.npy'
SAVE_PATH = './check_point'
CHECKPOINT_PATH = './check_point/checkpoint_owt.pt'

def main(config = config_owt):
    meta_config = config['model'].copy()
    meta_config.update(config['optimizer'])
    meta_config.update(config['train'])

    swanlab.init(
        project = f'demo_LM_owt{config['version']}',
